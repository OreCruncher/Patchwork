// From EnderIO - Public License
// https://github.com/SleepyTrousers/EnderIO/blob/master/enderio-base/src/main/java/crazypants/enderio/base/xp/XpUtil.java
 ...
 *
 * @see <a href=
 *      "https://github.com/OpenMods/OpenModsLib/blob/master/src/main/java/openmods/utils/EnchantmentUtils.java">OpenMods</a>
 *
 */
public class XpUtil {
	 ...
	 * 
	 * @param level
	 * @return The amount of XP a player needs to get from level 0 to the given
	 *         level
	 */
	...
	static {
		...
		for (int i = 0; i <= MAX_LEVEL; i++) {
			if (res < 0) {
				...
				Doodads.log().warn("Internal XP calculation is wrong. Level " + i + " already maxes out.");
			}
			xpmap[i] = res;
	 ...
	 * 
	 * @param level
	 * @return The amount of XP a player at the given level needs to get to the
	 *         <em>next</em> level
	 */
	 ...
	 * 
	 * @param player
	 * @return The total amount of experience (from both their level and their xp
	 *         bar) the given player has
	 */
	...
	public static void addPlayerXP(@Nonnull EntityPlayer player, int amount) {
		final int experience = Math.max(0, getPlayerXP(player) + amount);
