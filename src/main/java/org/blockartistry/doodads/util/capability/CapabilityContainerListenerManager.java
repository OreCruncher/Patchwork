...
/**
 * Manages the {@link IContainerListener}s that handle syncing of each item
 * capability.
 *
 * @author Choonster
 */
...
public class CapabilityContainerListenerManager {
	...
	private static class EventHandler {
		...
		/**
		 * Add the listeners to {@link EntityPlayer#inventoryContainer} when an
		 * {@link EntityPlayerMP} logs in.
		 *
		 * @param event
		 *                  The event
		 */
		...
		/**
		 * Add the listeners to a {@link Container} when it's opened by an
		 * {@link EntityPlayerMP}.
		 *
		 * @param event
		 *                  The event
